...
foreach ($name in $names) {
    ...
    try {
        ...
        $actionResults = @(); foreach ($index in $indexes) { $arguments = [object[]]@([string]$sequence, [int]$index); $stepTask = $runSingle.Invoke($runtime, $arguments); $actionResult = [string]$stepTask.GetAwaiter().GetResult(); $actionResults += $actionResult; if ($actionResult -match "^(Error|Failed|Fail)$") { $detail = $lastExecution.GetValue($runtime, $null); $rows = @($detail.Results | ForEach-Object { "$($_.StepName): status=$($_.Status), value=$($_.Value), comment=$($_.Comment)" }); throw "Safe action failed at STEP $($index + 1): $($sequenceDocument.StepList[$index].StepName). raw=$($detail.RawReturn), total=$($detail.TotalStatus), results=$($rows -join '; ')" } }
